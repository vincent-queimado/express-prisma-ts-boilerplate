...
export default async (userDatas: any) => {
    ...
    if (!required.success) return httpMsg.http401(required.msgError, errCode);

    // Check existing User
    const existUser = await getUser(datas.email);
    if (!existUser.success) return httpMsg.http401(existUser.msgError, errCode);

    // Check password
    const checkedPassword = await checkPassword(datas.password, existUser.data.password);
    if (!checkedPassword.success) return httpMsg.http401(checkedPassword.msgError, errCode);

    // // Generate token access
    const generatedToken = await generateToken(datas);
    if (!generatedToken.success) return httpMsg.http401(generatedToken.msgError, errCode);
